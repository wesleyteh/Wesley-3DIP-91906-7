...
import json, uuid, re, time, threading, queue, os, hashlib, binascii
...
class FoodItem:
    ...
    def to_dict(self):
        ...
        # ensure expiry serialised as iso
...
class User:
    def __init__(self, username, password, name, id=None, purchases=None):
        ...
        self.name = name
        self.id = id if id else str(uuid.uuid4())
        # purchases stored as list of FoodItem objects
...
def save_data():
    # Remove food items expired for more than 24 hours
    now = dt.datetime.now()
    cutoff = now - dt.timedelta(hours=24)
    # Remove from top-level food_items
    global food_items
    food_items = [f for f in food_items if isinstance(f.expiry, dt.datetime) and f.expiry > cutoff]
    # Remove from business listings
    for b in businesses:
...
for ru in raw.get("users", []):
    # convert purchases dicts to FoodItem
    purchases = []
...
class App(tk.Tk):
    ...
    def on_food_select(self, evt):
        ...
        if sel and hasattr(self, "visible_items") and sel[0] < len(self.visible_items):
    ...
    def _refresh_food_list(self):
        ...
        for item in food_items:
            ...
            if q and q not in item.name.lower() and q not in item.category.lower():
                continue
            visible.append(item)
        # store for quick lookup on double-click
        ...
        for it in visible:
            ...
            self.food_listbox.insert('end', f"{it.name} • ${it.price:.2f} • {it.category} • {seller} • {status}")
